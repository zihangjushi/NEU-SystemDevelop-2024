  ...
  methods: {
    ...
    /*async login() {
      this.$refs.loginFormRef.validate(async valid => {
        if (valid) {
          try {
            const response = await axios.post('http://localhost:8090/users/page', {
    ...
    async login() {
      this.$refs.loginFormRef.validate(async valid => {
        if (valid) {
          try {
            const response = await axios.post('http://localhost:8090/users/login', qs.stringify({
              username: this.loginForm.username,
              password: this.loginForm.password
            }), {
              headers: {
            ...
            if (result.code === '0') { // 假设成功的响应状态码为 '0'
              const token = result.data.token; // 假设后端返回的token字段为 token
            ...
            console.error('登录过程中出错:', error.response ? error.response.data : error.message);
