          ...
          <div style="display: flex; justify-content: space-between; width: 100%;">
            <div style="text-align: left;position: relative; top: 20px;">
              <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>修改会议</el-breadcrumb-item>
              </el-breadcrumb>
            </div>
        ...
        <el-card style="height: 630px; margin-top: 10px;">
          ...
          <el-form :model="form">
            <el-form-item label="会议标题" :label-width="formLabelWidth">
              <el-input v-model="form.meetingTitle" placeholder="请输入会议标题" autocomplete="off"/>
            </el-form-item>
...
const meetingId = route.params.meetingId; // 通过 props 获取 meetingId
...
const handleSubmit = async () => {
  ...
  if (listst.value !== null) {
    response = await axios.post('http://localhost:8070/api/upload', formData, {});
  }
  ...
  try {
    if (!response || (response && response.status === 200)) {
      ...
      const responseMeeting = await axios.put('http://localhost:8070/api/meetings/update', meetingData);
      //ElMessage.success(form.organizer);
      console.log(meetingData);
      if (responseMeeting.data.msg === "成功") {
        ElMessage.success("会议修改成功!");
        router.push('/meeting');
      } else {
        ElMessage.success("会议修改失败!请检查输入!");
      }
    } else {
...
const fetchMeetingData = async (meetingId) => {
  ...
  try {
    const response = await axios.get(`http://localhost:8070/api/meetings/${meetingId}`);
    const responseData = response.data;
    const data = responseData.data;
    console.log('Fetched meeting data:', data);
    form.meetingTitle = data.meetingTitle;
    //form.driverPhoto = data.driverPhoto;
    content.value = data.meetingDescription;
    form.organizer = data.organizer;
    // 设置开始日期（年月日）
    const startDate = new Date(data.startTime);
    value1.value = `${startDate.getFullYear()}-${(startDate.getMonth() + 1).toString().padStart(2, '0')}-${startDate.getDate().toString().padStart(2, '0')}`;
    // 设置起止时间（时分秒）
    const startTime = new Date(data.startTime);
    const endTime = new Date(data.endTime);
    value2.value = [startTime, endTime];
    // 如果有需要，可以将图片添加到 fileList 中
    const formats = ['jpg', 'png', 'jpeg'];
    let localImageUrl = null;
    for (let format of formats) {
      try {
        localImageUrl = require(`@/assets/${data.meetingTitle}.${format}`);
      } catch (e) {
        // Ignore the error and try the next format
      }
    }
    if (data.meetingTitle && localImageUrl) {
      fileList.value.push({
        name: data.meetingTitle,
        url: localImageUrl,
      });
    }
  } catch (error) {
